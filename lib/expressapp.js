      ...
      function update() {
        ...
        async.parallel([
            ...
            function(callback) {
              ...
              request(requestOptions, function(err, res1, body) {
                if (err || !body) {
                  log.error('can not connect coinmarketcap ' + err + ' ' + body);
                  return callback(err);
                }
                try {
                  var obj = JSON.parse(body);
                  if (obj.length && obj[0].price_btc) {
                    coinmarketcapRate = parseFloat(obj[0].price_btc);
                  }
                } catch (e) {
                  return callback(new Error('get eth market error ' + body + ',' + e));
                }
                if ((coinmarketcapRate > 0) && (coinmarketcapRate < 1)) {
                  return callback(null);
                } else {
                  return callback(new Error('get eth market error ' + body));
                }
              });
            }
          ],
