...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/v1/eth/address_nonce/:address', function(req, res) {
    getServerWithAuth(req, res, function(server) {
          var address = req.params['address'];
          server.getEthAddressNonce(address, function(err, nonce) {
              if (err) return returnError(err, res, req);
              var ret = {}
              ret.nonce = nonce;
              res.json(ret);
    ...
    router.get('/v1/try/address_nonce/:address', function(req, res) {
        getServerWithAuth(req, res, function(server) {
            var address = req.params['address'];
            server.getTriAddressNonce(address, function(err, nonce) {
                if (err) return returnError(err, res, req);
                var ret = {}
                ret.nonce = nonce;
                res.json(ret);
  ...
  router.get('/v1/rates/eth', function(req, res) {
    var server;
    try {
      server = getServer(req, res);
    } catch (ex) {
      ...
      function update() {
        ...
        request(requestOptions, function (err, res1, body) {
          if (err || !body) {
            log.error('bitpay rates request error ' + err + ' ' + res1 + ' ' + body)
            return;
          }
          try {
            ...
            log.error('bitpay rates parse error ' + body + ',' + e)
            return;
          }
          if (!bitPayRes || !bitPayRes.length) {
            log.error('bitpay rates length 0 ' + body)
            return;
          }
          var ethRate = 0;
          var i;
          for (i = 0; i < bitPayRes.length; i++) {
            if (bitPayRes[i].code === 'ETH') {
              ethRate = bitPayRes[i].rate
              break;
            }
          }
          if (!ethRate) {
            log.error('bitpay rates, not find eth rate ' + body)
            return;
          }
          ethRate = 1.0 / parseFloat(ethRate)
          for (i = 0; i < bitPayRes.length; i++) {
            bitPayRes[i].rate *= ethRate
            if (bitPayRes[i].code === 'ETH') {
              bitPayRes[i].rate = 1;
            }
          }
          server.storeEthPriceCache(bitPayRes, function (err) {
            if (err) {
              log.error('storeEthPriceCache, err', err);
            }
          });
        });
      }
    ...
    server.loadEthPriceCache(function (error, data) {
        if (error) {
          log.error('loadEthPriceCache, error', error);
          return returnError(error, res, req);
        } else {
    ...
    router.get('/v1/rates/try', function(req, res) {
      var server;
      try {
        server = getServer(req, res);
      } catch (ex) {
        log.error("can not get server")
        return returnError(ex, res, req);
      }
      function update() {
        var bitPayRes;
        var coinmarketcapRate = 0;
        async.parallel([
            function(callback) {
              var requestOptions = {
                url: 'https://bitpay.com/api/rates',
              ...
              request(requestOptions, function(err, res1, body) {
                if (err || !body) {
                  return callback(new Error('bitpay rates request error ' + err + ' ' + res1 + ' ' + body));
                }
                try {
                  var obj = JSON.parse(body);
                  bitPayRes = obj;
                } catch (e) {
                  return callback(new Error('bitpay rates parse error ' + body + ',' + e));
                }
            ...
            function(callback) {
              var requestOptions = {
                url: 'https://dncapi.bqiapp.com/api/coin/web-charts?code=trias&type=all&webp=1',
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json'
                },
              ...
              request(requestOptions, function(err, res1, body) {
                if (err || !body) {
                  log.error('can not connect TRY price server ' + err + ' ' + body);
                  return callback(err);
                }
                try {
                  var obj = JSON.parse(body);
                  if (obj.value.length) {
                    let value = obj.value
                    let arrayStart = value.lastIndexOf("[");
                    let arrayEnd = value.lastIndexOf("]")
                    if (arrayStart > 0 && arrayEnd > 0) {
                      let arrayString = value.substring(arrayStart, arrayEnd+1)
                      let arrayObj = JSON.parse(arrayString)
                      if (arrayObj.length === 5) {
                        coinmarketcapRate = arrayObj[2]
                      } else {
                        log.error("parse TRY price, array length not right" + arrayObj.length)
                      }
                    } else {
                      log.error("parse TRY price, not find array")
                    }
                  } else {
                    log.error("parse TRY price, not find value")
                  }

                } catch (e) {
                  return callback(new Error('get try market error ' + body + ',' + e));
                }
                if ((coinmarketcapRate > 0) && (coinmarketcapRate < 1)) {
                  log.info("get current try price", coinmarketcapRate)
                  return callback(null);
                } else {
                  log.error('get try market, price rate too large', coinmarketcapRate)
                  return callback(new Error('get try market error ' + body));
                }
              });
            }
          ],
          function(err, results) {
            if (err || !bitPayRes || !bitPayRes.length) {
              log.error("bitpay get fail, err", err)
              return;
            }
            for(var i=0; i<bitPayRes.length; i++) {
              bitPayRes[i].rate *= coinmarketcapRate;
            }
            bitPayRes.unshift({
              code: 'TRY',
              name: 'Trias Token',
            ...
            server.storeTRYPriceCache(bitPayRes, function (err) {
              if (err) {
                log.error('storeTRYPriceCache, err', err);
              }
            });
          });
      }
      ...
      setTimeout(update, 2*1000)
        server.loadTRYPriceCache(function (error, data) {
          if (error) {
            log.error('loadTRYPriceCache, error', error);
            return returnError(error, res, req);
          } else {
            return res.json(data);
          }
  ...
  router.get('/v1/get_transaction_pending_status', function(req, res) {
      var server = getServer(req, res);
      var q = req.query;
      var ret = {}
      if (!q.coin) {
        ret.code = 1
        ret.message = "can not find coin param"
        return res.json(ret);
      }
      q.coin = q.coin.toLowerCase()
      if(q.coin !== 'btc' &&  q.coin !== 'eth' && q.coin !== 'try') {
        ret.code = 1
        ret.message = 'coin param not supported: ' + q.coin
        return res.json(ret);
      }
      if (!q.network) {
        ret.code = 1
        ret.message = "can not find network param"
        return res.json(ret);
      }
      q.network = q.network.toLowerCase()
      if(q.network !== 'livenet' &&  q.network !== 'testnet') {
        ret.code = 1
        ret.message = 'network param not supported: ' + q.network
        return res.json(ret);
      }
      if (q.coin === 'eth' || q.coin === 'try') {
        q.network = 'livenet';
      }
      if (!q.txid) {
        ret.code = 1
        ret.message = "can not find txid param"
        return res.json(ret);
      }
      q.txid = q.txid.toLowerCase()
      server.checkTransactionInBlockchain(q.coin, q.network, q.txid, function (error, exist) {
        if (error) {
          ret.code = 1
          ret.message = '' + error
          return res.json(ret);
        ...
        ret.code = 0
        ret.status = exist ? "confirmed" : "pending"
        return res.json(ret);
      });
  });
