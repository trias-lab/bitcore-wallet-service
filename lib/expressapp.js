...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/v1/eth/address_nonce/:address', function(req, res) {
    ...
    router.get('/v1/tri/address_nonce/:address', function(req, res) {
        getServerWithAuth(req, res, function(server) {
            var address = req.params['address'];
  ...
  router.get('/v1/rates/eth', function(req, res) {
    ...
    router.get('/v1/rates/tri', function(req, res) {
        var bitPayRes;
        var coinmarketcapRate = 0;
        async.parallel([
                function(callback) {
                ...
                function(callback) {
                    ...
                    request(requestOptions, function(err, res1, body) {
                        ...
                        try {
                            ...
                            return callback(new Error('get tri market error ' + body + ',' + e));
                        }
                        if ((coinmarketcapRate > 0) && (coinmarketcapRate < 1)) {
                            log.info("get current tri price", coinmarketcapRate)
                            return callback(null);
                        } else {
                            log.error('get tri market, price rate too large', coinmarketcapRate)
                            return callback(new Error('get tri market error ' + body));
                        }
                    });
                }
            ],
            function(err, results) {
                ...
                if (err || !bitPayRes || !bitPayRes.length) {
                    log.error("bitpay get fail, use tri price in db cache, err", err)
