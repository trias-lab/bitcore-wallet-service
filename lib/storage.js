...
var tryCoin = require('bitcore-lib-tri');
...
var collections = {
  ...
  BTC_TXS_HISTORY: 'btc_txs_history',
  BTC_TXS_HISTORY_TESTNET: 'btc_txs_history_testnet',
  BTC_ADDRESS_TXIDS: 'btc_address_txids',
  BTC_ADDRESS_TXIDS_TESTNET: 'btc_address_txids_testnet',
  BTC_UTXO : 'btc_utxo',
...
Storage.prototype._createIndexes = function() {
  ...
  this.db.collection(collections.BTC_TXS_HISTORY).createIndex({
    txid: 1
  },{unique:true});
  this.db.collection(collections.BTC_TXS_HISTORY_TESTNET).createIndex({
    txid: 1
  },{unique:true});
  this.db.collection(collections.BTC_ADDRESS_TXIDS).createIndex({
    address: 1
  },{unique:true});
  this.db.collection(collections.BTC_ADDRESS_TXIDS_TESTNET).createIndex({
    address: 1
  },{unique:true});
  this.db.collection(collections.BTC_UTXO).createIndex({
    txid: 1,
    vout:1,
  },{unique:true});
  this.db.collection(collections.BTC_UTXO).createIndex({
  ...
  this.db.collection(collections.BTC_UTXO_TESTNET).createIndex({
    txid: 1,
    vout:1,
  },{unique:true});
  this.db.collection(collections.BTC_UTXO_TESTNET).createIndex({
...
Storage.prototype._completeTxData = function(walletId, txs, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(txs), function(tx) {
      ...
      var copayer = wallet.getCopayer(tx.creatorId);
      tx.creatorName = copayer ? copayer.name : 'unknown';
      _.each(tx.actions, function(action) {
        var copayer = wallet.getCopayer(action.creatorId);
        action.copayerName = copayer ? copayer.name : 'unknown';
      });
...
//return plain object
Storage.prototype.fetchAllAddress = function(coin, network, cb) {
  var self = this;
  if (!this.db) return cb();
...
//tmp
Storage.prototype.removeAllFiatRate = function(cb) {
  var self = this;
  self.db.collection(collections.FIAT_RATES).remove({
  }, {
    w: 1
  }, cb);
}
...
Storage.prototype.storeFiatRate = function(providerName, rates, cb) {
  ...
  */
  async.each(rates, function(rate, next) {
    self.db.collection(collections.FIAT_RATES).update({
      provider: providerName,
      code: rate.code,
    }, {
      '$set':{
...
Storage.prototype._completeTxNotesData = function(walletId, notes, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(notes), function(note) {
      var copayer = wallet.getCopayer(note.editedBy)
      note.editedByName = copayer ? copayer.name : 'unknown';
    });
...
Storage.prototype.getTryCheckHeight = function(network, cb) {
    var self = this;
    self.db.collection(collections.CACHE).findOne({
        type: 'tryCheckHeight',
        key: network
...
Storage.prototype.storeTryCheckHeight = function (network, height, cb) {
    var now = Date.now();
...
Storage.prototype.getTRYPriceCache = function(cb) {
  var self = this;
  self.db.collection(collections.CACHE).findOne({
    type: 'tryPriceCache',
  }, function(err, result) {
...
Storage.prototype.storeTRYPriceCache = function (data, cb) {
  var now = new Date()
...
Storage.prototype.batchGetBTCTxHistory = function (addressArray, isTestnet, cb) {
  var self = this
  var collection = isTestnet ? collections.BTC_TXS_HISTORY_TESTNET : collections.BTC_TXS_HISTORY
  var lookup = isTestnet ? collections.BTC_ADDRESS_TXIDS_TESTNET : collections.BTC_ADDRESS_TXIDS
  self.db.collection(lookup).find({
    address:{$in:addressArray}
  }).toArray(function (err, txidArrays) {
    if (err) {
      log.error('batchGetBTCTxHistory lookup address error', err)
      return cb(err)
    }
    if (!txidArrays || !txidArrays.length) {
      return cb(null, [])
    }
    var txidsArray = _.map(txidArrays, 'txids')
    var allTxids = _.flatten(txidsArray)
    var allTxidsUniq = _.uniq(allTxids)
    self.db.collection(collection).find({
      txid:{$in:allTxidsUniq}
    }).toArray(function (err, txArrays) {
      if (err) {
        log.error('batchGetBTCTxHistory get txs error', err)
        return cb(err)
      }
      log.info('batchGetBTCTxHistory result ' + txArrays.length + ', addressArray ' + addressArray)
      if (!txArrays || !txArrays.length) {
        return cb(null, [])
      }
      return cb(null, txArrays);
    })
  })
}

Storage.prototype.storeBTCTxHistory = function (address, tx, isTestnet, cb) {
  var self = this;
  var collection = isTestnet ? collections.BTC_TXS_HISTORY_TESTNET : collections.BTC_TXS_HISTORY
  var lookup = isTestnet ? collections.BTC_ADDRESS_TXIDS_TESTNET : collections.BTC_ADDRESS_TXIDS
  self.db.collection(collection).update(
    ...
    function (err, result) {
      if (err) {
        log.error('storeBTCTxHistory store tx error', err);
        return cb(err)
      }
      self.db.collection(lookup).update(
        {address:address},
        { $addToSet: {txids: tx.txid } },
        {upsert:true, w:1},
        function (err, result) {
          if (err) {
            log.error('storeBTCTxHistory store txid error', err);
            return cb(err)
          }
          return cb()
        }
...
Storage.prototype.getBTCUtxoWithAddress = function (address, isTestNet, cb) {
  var collection = isTestNet ? collections.BTC_UTXO_TESTNET : collections.BTC_UTXO
  this.db.collection(collection).findOne({
    address:address
  }, function(err, result) {
    if (err) return cb(err);
    return cb(null, result);
...
Storage.prototype.getBTCUtxoWithAddressArray = function (addressArray, isTestNet, cb) {
  var collection = isTestNet ? collections.BTC_UTXO_TESTNET : collections.BTC_UTXO
...
Storage.prototype.insertBTCUtxo = function(utxo, isTestNet, cb) {
  var collection = isTestNet ? collections.BTC_UTXO_TESTNET : collections.BTC_UTXO
  this.db.collection(collection).insertOne(utxo,{
      w: 1,
    }, cb);
};

Storage.prototype.removeBTCUtxo = function(utxo, isTestNet, cb) {
  var collection = isTestNet ? collections.BTC_UTXO_TESTNET : collections.BTC_UTXO
  var obj = {
    txid:utxo.txid,
    vout:utxo.vout,
  }
  this.db.collection(collection).remove(obj,{
    w: 1,
  }, cb);
};


Storage.prototype.loadBTCLatestBlockCache = function(isTestnet, cb) {
  var self = this;
  var type = isTestnet ? 'BTCLatestBlockCacheTestnet' : 'BTCLatestBlockCacheMainnet'
...
Storage.prototype.storeBTCLatestBlockCache = function (isTestnet, data, cb) {
  var now = new Date()
  var type = isTestnet ? 'BTCLatestBlockCacheTestnet' : 'BTCLatestBlockCacheMainnet'
...
Storage.prototype.updateBTCLatestBlockCacheState = function (isTestnet, state, cb) {
  var now = new Date()
  var type = isTestnet ? 'BTCLatestBlockCacheTestnet' : 'BTCLatestBlockCacheMainnet'
