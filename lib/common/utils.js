 ...
 */
Utils.scientificNotationToString = function(param) {
...
Utils.parseBalance = function(amount) {
  var value;
  var s = amount.toString();
  if (s.substring(0,2) === '0x' || s.substring(0,2) === '0X') {
    value = parseInt(s)
  } else {
    value = parseFloat(s)
  }
...
Utils.makeHttpsGetRequest = function(host, path, timeout, callback) {
  ...
  var newReq = https.request(opt, function(newRes){
    if(newRes.statusCode === 200){
      ...
      callback('makeHttpsGetRequest, status:' + newRes.statusCode)
    }
  });
  newReq.on('error', (e) => {
    callback('makeHttpsGetRequest: error ' + e)
  });
  newReq.on('timeout', () => {
    newReq.abort();
  });
