    ...
    _.each(_.values(['eth']), function(coin) {
        _.each(_.values(['livenet']), function(network) {
          ...
          self.storage.storetEthCheckHeight(network, 0, function (err, result) {
            if (err) {
              logError('tmp clean eth check height, err ' + err);
              return;
            }
...
BlockchainMonitorEth.prototype._checkBlockHeight = function(coin, network, explorer) {
  ...
  self.storage.getEthCheckHeight(network, function (err, lastHeight) {
      if (err) {
          logError('Could not fetch eth check height from the db');
          nextLoop();
          return;
      }
      explorer.getBlockchainHeight(function (err, curHeight) {
          if (err || !curHeight) {
              ...
              logError('getBlockchainHeight error ' + curHeight + ',' + err + ', nextLoopDelay seconds:' + parseInt(nextLoopDelay/1000), new Date());
              nextLoop();
              return;
          }
          ...
          explorer.getBlockchainData(lastHeight+1, function (err, blockData) {
              if (err || !blockData) {
                  nextLoopDelay = longDelay;
                  logError('getBlockchainData error, lastHeight+1:' + (lastHeight+1) + ', curHeight:' + curHeight + 'error:' + err, 'blockData', blockData);
                  nextLoop();
                  return;
              }
              nextLoopDelay = shortDelay
              self.storage.storetEthCheckHeight(network, lastHeight+1, function (err, result) {
                  if (err) {
                      logError('storetEthCheckHeight, err ' + err);
                      nextLoop();
                      return;
                  }
                  ...
                  for (i = 0; i < blockData.transactions.length; i++) {
                      (function (index) {
                          ...
                          self.storage.findAddresses('eth', checkAddressArray, function (err, result) {
                              if (err) {
                                  logError('findAddresses eth err ' + [transaction.from, transaction.to]);
                              } else {
                                  if (result && result.length) {
                                      ...
                                      self.storage.storeETHTxHistory(transaction, function (error) {
                                          if (error) {
                                              logError('monitor storeETHTxHistory err ' + error);
                                          }
                                      });
...
BlockchainMonitorEth.prototype._handleIncomingPayments = function(coin, network, transaction) {
  ...
  if (!transaction || !transaction.to) {
    logError('_handleIncomingPayments not find transaction.to');
    return;
  }
  ...
  self.storage.fetchAddressByCoin(coin, transaction.to, function(err, address) {
      ...
      if (!address) {
        logError('_handleIncomingPayments not find address')
        return;
      }
      if (address.isChange) {
...
BlockchainMonitorEth.prototype._handleIncomingPaymentsWithAddress = function(coin, network, addr, value, hash) {
  var self = this;
  if (!addr || !value) {
