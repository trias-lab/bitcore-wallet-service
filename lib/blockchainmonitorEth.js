...
BlockchainMonitorEth.prototype._checkBlockHeight = function(coin, network, explorer) {
  ...
  self.storage.getEthCheckHeight(network, function (err, lastHeight) {
      ...
      explorer.getBlockchainHeight(function (err, curHeight) {
          if (err || !curHeight) {
              log.error('getBlockchainHeight error ' + curHeight + ',' + err);
              return;
          }
          ...
          self.storage.storetEthCheckHeight(network, curHeight, function (err, result) {
              if (err) {
              ...
              explorer.getBlockchainData(curHeight, function (err, blockData) {
                  if (err) {
                      log.error('getBlockchainData error ' + curHeight + ',' + err);
                      return;
                  }
                  ...
                  for (i = 0; i < blockData.transactions.length; i++) {
                      ...
                      (function(from, to) {
                          self.storage.findAddresses('eth', [from, to], function (err, result) {
                              if (err) {
                                  log.error('findAddresses eth err ' + [from, to]);
                              } else {
                                  if (result && result.length) {
                                      log.info('find my transaction ' + typeof result + ', length ' + result.length);
                      ...
                      })(_.clone(transaction.from), _.clone(transaction.to));
                  }
              });
          });
