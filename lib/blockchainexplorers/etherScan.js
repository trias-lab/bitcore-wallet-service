...
EtherScan.prototype.getAddressActivity = function(address, cb) {
    ...
    makeGetRequest(path, function (err, obj) {
        if (err) {
            ...
            var number = parseFloat(obj.result)
        ...
        makeGetRequest(path, function (err, obj) {
            if (err) {
                ...
                for (var j = 0; j < array.length; j++) {
                    var balance = parseFloat(array[j].balance);
                    if (isNaN(balance)) {
                        log.error('getMultiAddressBalance request balance parse error ' + array[j].balance);
                        balance = 0;
                    }
                    resultMap[array[j].account] = parseFloat(balance);
                }
                return next();
            }
        });
...
EtherScan.prototype.getTransactions = function(addresses, from, to, cb, walletId, storage) {
    ...
    function  ethTxToBitTx(ethTx) {
        ...
        bitTx.vin[0].valueSat = parseFloat(ethTx.value) + realFee;
        bitTx.vout = [{
            scriptPubKey:{
                addresses:[]
            }
        ...
        bitTx.vout[0].value = parseFloat(ethTx.value);
        ...
        bitTx.size = 225;
        bitTx.valueIn = parseFloat(ethTx.value) + realFee;
        ...
        self.getTxList(address, function (err, ethTxs) {
            if(err) {
                ...
                if (!ethTxs || !ethTxs.length) {
                  log.info('eth get txlist empty, address ' + address + ', api index ' + (apiIndex-1));
                  return next();
                }
