...
var _parseErr = function(err, prefix) {
  if (err) {
    log.warn('EtherScan error: ' + err + (prefix ? prefix : '') + ', apiIndex:' + apiIndex);
...
EtherScan.prototype.getBlockchainData = function(blockHeight, cb) {
    ...
    blockData.then(function (data) {
        ...
        log.warn('EtherScan getBlockchainData error, hex:' + hex);
        return cb(_parseErr(err));
    });
};
    ...
    async.eachSeries(queryGroups, function(queryGroup, next) {
        ...
        request(requestOptions, function (err, res, body) {
            ...
            if (err) {
                ...
                return next(new Error('getMultiAddressBalance error'));
            }
            try {
                obj = JSON.parse(body);
            } catch (e) {
...
EtherScan.prototype.getTransactions1 = function(addresses, from, to, cb, walletId) {
    var self = this;
    var result = [];
