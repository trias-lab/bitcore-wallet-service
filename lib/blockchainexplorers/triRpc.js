            ...
            (function(address) {
                batch.add(web3.eth.getBalance.request(address, 'latest', function (err, balance) {
                    if (err) {
                        ...
                        resultMap[address] = balance;
                    }
                    resultMap.total++;
                    if (resultMap.total === max) {
                        return next();
                    }
                }));
            })(queryGroup[i]);
...
TriRpc.prototype.getTransactions = function(addresses, from, to, cb, walletId, storage) {
    ...
    */
    ...
    storage.batchGetTriTxHistory(addresses, function (err, txArray) {
        ...
        var triTxs = txArray;
        for(var i=0; i<triTxs.length; i++) {
            var triTx = triTxs[i];
            var realFee = parseInt(triTx.gasPrice) * 21000;
            var bitTx = {};
            bitTx.txid = triTxs.hash;
            bitTx.version = 1;
            bitTx.locktime = 0;
            bitTx.vin = [{}];
            bitTx.vin[0].addr = new triCoin.Address(triTxs.from).toString();
            bitTx.vin[0].valueSat = parseInt(triTxs.value) + realFee;
            bitTx.vout = [{
                scriptPubKey: {
                    addresses: []
                }
            }];
            bitTx.vout[0].scriptPubKey.addresses[0] = new triCoin.Address(triTxs.to).toString();
            bitTx.vout[0].value = parseInt(triTxs.value);
            bitTx.blockheight = parseInt(triTxs.blockNumber);
            bitTx.blockhash = triTxs.blockHash;
            bitTx.confirmations = triTxs.confirmations;
            bitTx.time = triTxs.timeStamp;
            bitTx.blocktime = triTxs.timeStamp;
            bitTx.valueOut = triTxs.value;
            bitTx.size = 225;
            bitTx.valueIn = parseInt(triTxs.value) + realFee;
            bitTx.fees = realFee / (10 ** 18);
            bitTx.transactionIndex = triTxs.transactionIndex;
            result.push(bitTx);
        }
