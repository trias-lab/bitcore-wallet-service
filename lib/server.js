...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!Utils.checkValueInCollection(opts.coin, Constants.COINS)) {
    log.error('invalid coin ', opts.coin)
  ...
  async.series([
    ...
    function(acb) {
      ...
      self.storage.storeWallet(wallet, function(err) {
        self.logi('Wallet created', wallet.id, opts.network, opts.coin);
        newWallet = wallet;
        return acb(err);
      });
    }
...
WalletService.prototype._getUtxos = function(coin, addresses, cb) {
  ...
  self.logi('Querying utxos: coin %s, %s addrs, addrs[0]:%s', coin, addresses.length, addresses[0]);

  bc.getUtxos(addresses, function(err, utxos) {
    if (err) return cb(err);

    var utxosNew = _.map(utxos, function(utxo) {
      var u = _.pick(utxo, ['txid', 'vout', 'address', 'scriptPubKey', 'amount', 'satoshis', 'confirmations']);
      u.confirmations = u.confirmations || 0;
      u.locked = false;
      self.logi('_getUtxos coin %s, address:%s, satoshis %s(%d), amount %s', coin, u.address, u.satoshis, _.isNumber(u.satoshis), u.amount);
      if(!_.isNumber(u.satoshis)) {
        self.logi('utxo parse error, u.satoshis should be number')
      }
      var multiply = (coin === 'btc' || coin === 'bch') ? 1e8 : 1e18;
      u.satoshis = _.isNumber(u.satoshis) ? +u.satoshis : Utils.strip(u.amount * multiply);
      delete u.amount;
      //self.logi('Querying utxos result coin %s, address:%s, satoshis %s', coin, u.address, u.satoshis);
      return u;
    });

    return cb(null, utxosNew);
  }, self.walletId, self.storage);
};
...
WalletService.prototype.getFeeLevels = function(opts, cb) {
  ...
  function storeFeeLevelsCache(values,next) {
   ...
   if (opts.coin === 'eth' || opts.coin === 'try') {
       var feeLevels = Defaults.FEE_LEVELS[opts.coin];
        var fees = _.map(feeLevels, function(level) {
            var result = {
                level: level.name,
            };
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    if (err) return cb(err);
    if (utxos.length && utxos[0].address.indexOf("0x") === 0) {
      var gas = txp.feePerKb * 21000;
      var source = null;
      self.logi('eth feePerKb:' + txp.feePerKb + ', txp total ' + txp.getTotalAmount());
      for(var i=0; i<utxos.length; i++) {
         var utxo = utxos[i];
         self.logi('utxo ' + utxo.satoshis);
         if (utxo.satoshis > txp.getTotalAmount() + gas) {
           source = utxo;
           break;
         }
      }
      ...
      txp.setInputs([source]);
      txp.fee = gas;
      self.logi('Successfully built transaction. Total fees: ' + txp.fee);
      return cb();
    }
    ...
    self.getWallet({}, function(err, wallet) {
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          function(next) {
            if (opts.sendMax) return next();
            if (wallet.coin === 'eth' || wallet.coin === 'try') return next();
          ...
          function(next) {
            ...
            self._getFeePerKb(wallet, opts, function(err, fee) {
              feePerKb = fee;
              self.logi("_getFeePerKb feePerKb=" +feePerKb);
              next();
            });
          },
          function(next) {
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        if (wallet.coin === 'eth' || wallet.coin === 'try') {
...
WalletService.prototype.getEthAddressNonce = function(address, cb) {
    var bc = this._getBlockchainExplorer('eth', 'livenet');
    if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
    bc.getAddressTransactionCount(address, function(err, count) {
        if (err) return cb(err);
        return cb(null, count);
    });
};

WalletService.prototype.getTriAddressNonce = function(address, cb) {
    var bc = this._getBlockchainExplorer('try', 'livenet');
    if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
    bc.getAddressTransactionCount(address, function(err, count) {
        if (err) return cb(err);
        return cb(null, count);
...
WalletService.prototype._processBroadcast = function(txp, opts, cb) {
  var self = this;
  self.logi('_processBroadcast  txp', txp)
  $.checkState(txp.txid);
  opts = opts || {};
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      self._broadcastRawTx(wallet.coin, wallet.network, raw, function(err, txid) {
        self.logi('_broadcastRawTx err', err, 'coin', wallet.coin, 'txid', txid)
        if (wallet.coin === 'eth' || wallet.coin === 'try') {
          txp.txid = txid;
        }
        if (err) {
...
WalletService.prototype._normalizeTxHistory = function(txs, coin) {
  var now = Math.floor(Date.now() / 1000);
  ...
  return _.map([].concat(txs), function(tx) {
    ...
    var outputs = _.map(tx.vout, function(item) {
      ...
      var amount = parseInt((item.value * 1e8).toFixed(0));
      if (coin  && (coin === 'eth' || coin === 'try')) {
        amount = item.value;
      }
      return {
        address: itemAddr,
    ...
    var fee = parseInt((tx.fees * 1e8).toFixed(0));
    if (coin && (coin === 'eth' || coin === 'try')) {
        fee = parseInt((tx.fees * 1e18).toFixed(0));
    }
...
WalletService.prototype._getBlockchainHeight = function(coin, network, cb) {
  ...
  var heightCacheSeconds = Defaults.BLOCKHEIGHT_CACHE_TIME;
  if (coin === 'eth') {
      heightCacheSeconds = Defaults.BLOCKHEIGHT_CACHE_TIME_ETH;
  } else if (coin === 'try') {
      heightCacheSeconds = Defaults.BLOCKHEIGHT_CACHE_TIME_TRI;
  }
  if (!cache.current || (now - cache.updatedOn) > heightCacheSeconds * 1000) {
  ...
  function getNormalizedTxs(wallet, addresses, from, to, cb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        self.logi('Querying tx for: %s addrs, coin %s', addresses.length, wallet.coin);

        bc.getTransactions(addressStrs, from, to, function(err, rawTxs, total) {
          if (err) return next(err);

          txs = self._normalizeTxHistory(rawTxs, wallet.coin);
          totalItems = total;
          return next();
        }, self.walletId, self.storage);
      },
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.storage.fetchAddresses(self.walletId, function(err, addresses) {
      ...
      async.waterfall([
        ...
        function(txs, next) {
          /*
          self.logi('getTxHistory step1 txs:', txs);
          for(var i=0; i<txs.items.length; i++) {
              var item = txs.items[i];
              for(var j=0; j<item.inputs.length; j++) {
                  self.logi('item ' + i + ' input ' + j, item.inputs[j]);
              }
              for(var j=0; j<item.outputs.length; j++) {
                  self.logi('item ' + i + ' output ' + j, item.outputs[j]);
              }
          }
          */
          if (!txs.items.length) {
        ...
        if (err) return cb(err);
        if (!res.txs.items || !res.txs.items.length) {
            return cb(null, [], false);
        }
        /*
        self.logi('getTxHistory step2 res:', res);
          for(var i=0; i<res.txs.items.length; i++) {
              var item = res.txs.items[i];
              for(var j=0; j<item.inputs.length; j++) {
                  self.logi('item ' + i + ' input ' + j, item.inputs[j]);
              }
              for(var j=0; j<item.outputs.length; j++) {
        ...
        */
        ...
        //self.logi('getTxHistory step3 finalTxs:', finalTxs);
        tagLowFees(wallet, finalTxs, function(err) {
          if (err)
            self.logw('Failed to tag unconfirmed with low fee');

          //self.logi('getTxHistory step4 finalTxs:', finalTxs);
        ...
        self.storage.storeWallet(wallet, function(err) {
          ...
          async.eachSeries(derivators, function(derivator, next) {
            ...
            self.storage.fetchWallet(wallet.id, function(err, wallet) {
              ...
              wallet.scanStatus = 'success';
              if (error) {
                self.logi('wallet scan error', error);
              }
...
WalletService.prototype.loadEthPriceCache = function(cb) {
  var self = this;
  self.storage.getEthPriceCache(cb);
};

WalletService.prototype.storeEthPriceCache = function(data, cb) {
    var self = this;
    self.storage.storeEthPriceCache(data, cb);
}

WalletService.prototype.loadTRYPriceCache = function(cb) {
  var self = this;
  self.storage.getTRYPriceCache(cb);
};

WalletService.prototype.storeTRYPriceCache = function(data, cb) {
  var self = this;
  self.storage.storeTRYPriceCache(data, cb);
}

WalletService.prototype.checkTransactionInBlockchain = function(coin, network, txid, cb) {
  var self = this
  if (!txid || !coin || !network) return cb(new Error('param empty'));
  var bc = this._getBlockchainExplorer(coin, network);
  if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
  bc.getTransaction(txid, function(err, tx) {
    if (err) return cb(err);
    //self.logi('checkTransactionInBlockchain tx:', tx);
    if (tx && tx.blockHash && tx.blockHash.length) {//eth try
      return cb(null, true);
    } else if (tx && tx.block && tx.block > 0) {//btc
      return cb(null, true);
    } else {
      return cb(null, false);
    }
