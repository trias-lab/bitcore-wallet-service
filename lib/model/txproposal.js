...
TxProposal.prototype._buildTx = function() {
  ...
  if (self.coin !== 'eth' || self.coin !== 'try') {
...
TxProposal.prototype.getRawTx = function() {
  if (this.coin === 'eth' || this.coin === 'try') {
      var sigs = this._getCurrentSignatures();
      if (sigs && sigs.length) {
          var signatures = sigs[0].signatures
          var raw = signatures[0][0].signature;
          return raw;
      }
      return null;
  }
  ...
  try {
    if (this.coin === 'eth' || this.coin === 'try') {
        ...
        if (this.status === 'accepted') {
            this.raw = signatures[0][0].signature;
            //this.txid = tx.id;
        }
        return true;
    }
