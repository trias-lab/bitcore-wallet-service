...
BlockchainMonitorTri.prototype._checkBlockHeight = function(coin, network, explorer) {
    ...
    self.storage.getTriCheckHeight(network, function (err, lastHeight) {
        ...
        explorer.getBlockchainHeight(function (err, curHeight) {
            if (err || !curHeight) {
                nextLoopDelay = 1000 * 20;
                log.error('getBlockchainHeight error ' + curHeight + ',' + err + ', nextLoopDelay seconds:' + parseInt(nextLoopDelay / 1000));
                nextLoop();
                return;
            }
            nextLoopDelay = 100
            ...
            explorer.getBlockchainData(lastHeight + 1, function (err, blockData) {
                if (err || !blockData) {
                    log.error('getBlockchainData error, lastHeight+1:' + (lastHeight+1) + ', curHeight:' + curHeight + 'error:' + err);
                    nextLoop();
                    return;
                }
                self.storage.storetTriCheckHeight(network, lastHeight + 1, function (err, result) {
                    ;
                    if (err) {
                    ...
                    log.info('handle new block data, height:' + (lastHeight + 1))
                    if (!blockData.transactions.length) {
                        if ((parseInt(lastHeight) % 100) === 0) {
                            log.info('empty block, height ', lastHeight);
                        }
                        return;
                    }
                    ...
                    for (i = 0; i < blockData.transactions.length; i++) {
                        (function (index) {
                            ...
                            transaction.from = new triCoin.Address(transaction.from).toString();
                            transaction.to = new triCoin.Address(transaction.to).toString();
                            ...
                            self.storage.findAddresses('tri', checkAddressArray, function (err, result) {
                                if (err) {
                                    log.error('findAddresses tri err ' + [transaction.from, transaction.to]);
                                } else {
                                    if (result && result.length) {
                                        ...
                                        self.storage.storeTriTxHistory(transaction, function (error) {
                                            if (error) {
                                                log.error('monitor storeTriTxHistory err ' + error);
                                            }
                                        });
                                    }
                                }
