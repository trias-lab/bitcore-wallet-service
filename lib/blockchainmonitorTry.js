  ...
  self.storage.getTryCheckHeight(network, function (err, lastHeight) {
    ...
    explorer.getBlockchainHeight(function (err, curHeight) {
      ...
      explorer.getEventsAndHeaderInBlock(lastHeight+1, function (err, events, blockHeader) {
        ...
        if ((events && events.length) && (!blockHeader || !(blockHeader.timestamp))) {
          nextLoopDelay = 1000 * 3;
          logError('getBlockchainData, not get blockHeader, lastHeight+1:' + (lastHeight+1) + ', curHeight:' + curHeight + 'error:' + err);
          nextLoop();
          return;
        }
        nextLoopDelay = 1000 * 5
